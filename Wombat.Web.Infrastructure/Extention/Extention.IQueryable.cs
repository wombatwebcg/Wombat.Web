...
namespace Wombat.Web.Infrastructure
{
    ...
    public static partial class Extention
    {
        ...
        /// <param name="pageInput">分页参数</param>
        /// <returns></returns>
        public static async Task<PageResult<T>> GetPageResultAsync<T>(this ISelect<T> source, PageInput pageInput)
        {
            var count = await source.CountAsync();
            string orderby = "\"" + pageInput.SortField.ToString() + "\"" + pageInput.SortType.ToString();
            var list = await source.OrderBy(orderby)
                .Skip((pageInput.PageIndex - 1) * pageInput.PageRows)
                .Take(pageInput.PageRows)
                .ToListAsync();

            return new PageResult<T> { Data = list, Total = (int)count };
        }
        ...
        /// <param name="pageInput">分页参数</param>
        /// <returns></returns>
        public static async Task<List<T>> GetPageListAsync<T>(this ISelect<T> source, PageInput pageInput)
        {
